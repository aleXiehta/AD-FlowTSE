...
class LightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # alpha_grid = torch.tensor([alpha.item(), 1.0], device=source.device)
        alpha_grid = torch.tensor([alpha.mean().item(), 1.0], device=source.device)
    ...
    #     # Only step the scheduler after epoch 10
    #     if self.current_epoch >= self.config['scheduler']['start_epoch']:
